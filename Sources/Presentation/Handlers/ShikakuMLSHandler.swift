...
public final class ShikakuMLSHandler {
    ...
    {
        guard !memberPublicKeys.isEmpty else { throw HandlerError.invalidArgument }
    ...
    {
        guard !cohortID.isEmpty, !newMemberPublicKey.isEmpty, !senderPrivateKey.isEmpty else {
    ...
    // Cohort状態取得
    public func getCohortState(cohortID: String) async throws -> GetCohortStateResponseDTO? {
    ...
    // 鍵生成
    public func generateKeyPair(participantID: String) async throws -> GenerateKeyPairResponseDTO? {
        guard !participantID.isEmpty else { throw HandlerError.invalidArgument }
        try await keyManagementController.generateKeyPair(participantID: participantID)
    ...
    public func sendMessage(
        ...
        guard !plaintext.isEmpty, !senderPrivateKey.isEmpty, !recipientPublicKey.isEmpty else {
            throw HandlerError.invalidArgument
        }
    ...
    public func decryptMessage(
        ...
        guard !ciphertext.isEmpty, !senderPublicKey.isEmpty, !recipientPrivateKey.isEmpty else {
            throw HandlerError.invalidArgument
        }
